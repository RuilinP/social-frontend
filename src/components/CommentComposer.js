...
function CommentComposer({ postId, user }) { 
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        ...
        const response = await fetch(`http://34.118.227.55:5000/comments/post/${postId}`, {
          method: "GET",
          headers: {
  ...
  const fetchLikeCount = async (commentId) => {
    try {
      const res = await fetch(`http://34.118.227.55:5000/comments/${commentId}/count`);
  ...
  const handleLike = async (commentId) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://34.118.227.55:5000/comments/${commentId}/like`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
