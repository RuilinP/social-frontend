...
function PostFeed({ user, refresh }) {
  ...
  useEffect(() => {
    fetch("http://34.118.227.55:5000/posts")
      .then((res) => res.json())
      .then((data) => {
        setPosts(data);
  ...
  const fetchLikeCount = (postId) => {
    fetch(`http://34.118.227.55:5000/posts/${postId}/count`)
      .then((res) => res.json())
  ...
  const handleLike = (postId) => {
    fetch(`http://34.118.227.55:5000/posts/${postId}/like`, {
      method: "POST",
      headers: {
            ...
            {post.PostImages?.length > 0 && (
              <div className="d-flex flex-wrap">
                {post.PostImages.map((img, index) => (
                  <Image
                    key={index}
                    src={`http://34.118.227.55:5000${img.imageUrl}`}
                    thumbnail
                    className="me-2 mb-2"
